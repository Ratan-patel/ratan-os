...
cp -f "$ISO" artifacts/ratan-os-amd64.iso
sha256sum artifacts/ratan-os-amd64.iso | tee artifacts/SHA256SUMS
stat -c 'ISO bytes: %s\nISO MiB: %.1f' artifacts/ratan-os-amd64.iso
if (( $(stat -c '%s' artifacts/ratan-os-amd64.iso) > 2000000000 )); then
  echo "FAIL: ISO 2 GB limit se bada hai." >&2
  exit 4
fi

echo "ISO taiyar hai: artifacts/ratan-os-amd64.iso"
